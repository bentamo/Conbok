...
function conbook_event_description_shortcode($atts) {
    ...
    $content = wpautop($event->post_content);
    if (!$content) return '';

    // Wrap content without interfering with <p> tags
    $output = '<section class="event-description">';
    $output .= '<h3 class="event-description-title">Description</h3>';
    $output .= '<div class="event-description-content">' . $content . '</div>';
    $output .= '</section>';
