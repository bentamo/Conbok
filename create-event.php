...
add_shortcode('create-event', function ($atts = []) {
    ...
    (function(){
        ...
        const addBtn = root.querySelector('.add-ticket-btn');
        ...
        function addTicket() {
            ...
            item.innerHTML = `
                <input type="text" name="ticket_name_${idx}" placeholder="Ticket Name" required />
                <input type="number" name="ticket_price_${idx}" placeholder="Price" min="0" step="0.01" required />
                <button type="button" class="remove-ticket">✖</button>`;
            ticketsList.appendChild(item);
            item.querySelector('.remove-ticket').addEventListener('click', () => item.remove());
        }
        addTicket(); // add one ticket by default
