<?php
add_shortcode('create-event-form', function ($atts = []) {
    $uid = uniqid('event-');
    ...
    <div class="event-form-container">
        ...
        <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>" enctype="multipart/form-data" id="<?php echo esc_attr($uid); ?>-form">
            <!-- Hidden action field for WP admin-post handling -->
            ...
            <div class="event-form-wrapper">
                ...
                <div id="<?php echo esc_attr($uid); ?>" class="event-form card">
                    ...
                    <!-- Allows dynamic addition of ticket types -->
                    <div>
    ...
    <style>
        ...
        .image-slot .remove-image-btn:hover {
    ...
    <script>
        (function() {
            // Grab main form and root container by unique ID
            const form = document.getElementById('<?php echo esc_js($uid); ?>-form');
            const root = document.getElementById('<?php echo esc_js($uid); ?>');
            const ticketsList = root.querySelector('.tickets-list'); // Container for ticket items
            const addBtn = root.querySelector('.add-ticket-btn');   // Button to add a new ticket
            const ticketError = root.querySelector('.ticket-error'); // Error message if no tickets
            const backBtn = document.querySelector('.back-btn');    // Optional back button

            let ticketIndex = 0; // Tracks ticket numbering

            // Create a new ticket input item
            function createTicketItem(index) {
                const item = document.createElement('div');
                item.className = 'ticket-item';
                item.innerHTML = `
                    <input type="text" name="ticket_name_${index}" placeholder="Ticket Name" required />
                    <input type="number" name="ticket_price_${index}" placeholder="Price" min="0" step="0.01" required />
                    <button type="button" class="remove-ticket">✖</button>`;

                // Remove ticket when clicking the remove button
            ...
            function addTicket() {
                ticketIndex++;
                const ticketItem = createTicketItem(ticketIndex);
                ticketsList.appendChild(ticketItem);
            }

            // Validate tickets on form submission
            form.addEventListener('submit', e => {
                if (!ticketsList.querySelector('.ticket-item')) {
                    e.preventDefault(); // Prevent submission if no tickets
            ...
            // Handle image upload, preview, and removal
            const imgSlot = form.querySelector('.image-slot');
            if (imgSlot) {
                const fileInput = imgSlot.querySelector('input[type=file]');
                const previewImg = imgSlot.querySelector('img');
                const uploadText = imgSlot.querySelector('.upload-text');
            ...
            // Initialize with one ticket and bind add button
            addTicket();
